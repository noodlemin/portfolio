...
const Screenshot = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${show ? ` <div class="fixed inset-0px top-51px bg-[#000000dd] col-center p-50px"> <div class="w-full w-100% md:w-80% lg:w-70% col text-center gap-5 bg-[var(--main)] border-solid border-1px border-[var(--border)] p-5 rounded-xl"><div class="aspect-video col bg-contain w-100% rounded-xl bg-no-repeat bg-contains bg-center"${add_attribute("style", `background-image: url(${screenshot?.src});`, 0)}><p class="font-italic m-t-auto m-x-auto m-b-5 inline-flex bg-[var(--main-60)] border-solid border-1px border-[var(--border)] p-x-5 p-2 rounded-xl">${escape(screenshot?.label)}</p></div> <p class="text-[var(--accent-text)] text-0.7em" data-svelte-h="svelte-hpptvy">Click outside the frame to close</p></div></div>` : ``}`;
});
